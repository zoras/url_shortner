class ShortUrlsController < ApplicationController
  ...
  def redirect
    short_url_id = params[:id]
    short_url = ShortUrl.find_by(shortened_url: short_url_id)
    if short_url
      short_url.increment_visit
      redirect_to short_url.long_url
    else
      respond_to do |format|
        format.html { redirect_to short_urls_url, alert: 'Couldn\'t not find url.' }
      end
    end
  end
  ...
  private
    ...
    def short_url_params
      params.require(:short_url).permit(:long_url, :shortened_url)
    end
end
